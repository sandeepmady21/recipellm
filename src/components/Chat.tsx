...
interface Message {
  ...
  isUser: boolean;
  timestamp: string;
  sqlCode?: string;  // SQL code generated by LLM
    ...
    try {
      ...
      if (response.success) {
        ...
        if (results && Array.isArray(results)) {
          messageContent += results.map(recipe => 
            `\n• ${recipe.name} (ingredients: ${recipe.ingredients.join(', ')})`
          ).join('');
        }
        
        // Add simulated SQL and raw response for the dev view
        sqlCodeContent = "SELECT * FROM recipes WHERE " + 
          (userMessage.content.toLowerCase().includes("ingredient") ? 
           "ingredients LIKE '%query_terms%'" : 
           "1=1");
        
        rawResponse = `Query processed: "${userMessage.content}"\n` +
          `Database operation: ${JSON.stringify(response)}\n` +
          `Found ${response.data?.length || 0} recipes matching criteria.`;
      } else {
